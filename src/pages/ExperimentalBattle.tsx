...
export const ExperimentalBattle = ({ levels, initialLevelIndex = 0, isEndless, gameMode, inventory, setInventory, onFinish }: BattleProps) => {
    ...
    if (gameState.playerHP > 0 && gameState.enemyHP > 0 && !showVictory && !showDefeat && !showInventory) {
      const timer = setInterval(() => {
        setEnemyCooldown(prev => {
          ...
          // Scaling cooldown speed by enemy tier
  ...
  const handleAnswer = (answer: string) => {
    ...
    setTimeout(() => {
      ...
      setGameState(prev => {
        ...
        // Level Progression check moved to a separate check if needed, 
        // but here we just increment the word index regardless of enemy death.
        // If enemy died, it will be handled by the useEffect.
  ...
  // Centralized Enemy Death and Level Progression Logic
  useEffect(() => {
    if (gameState.enemyHP <= 0 && !showVictory && !showDefeat) {
      setGameState(prev => {
        ...
        // We don't automatically increment word index here anymore 
        // to avoid double skipping when answering correctly.
        let nextWordIdx = prev.currentWordIndex;
        
        // Check level progression (if not endless)
        // If the index was incremented elsewhere and pushed past the limit
        if (!isEndless) {
          if (nextWordIdx >= activeWords.length) {
  ...
  }, [gameState.enemyHP, isEndless, levels.length, currentLevelIndex, activeWords.length, showVictory, showDefeat]);
  ...
  const useAttackSkill = () => {
    if (skillPoints >= 30 && gameState.enemyHP > 0) {
      ...
      setGameState(prev => ({ 
        ...prev, 
        enemyHP: newEnemyHP,
        // If skill kills the enemy, skip the current word as requested
  ...
  return (
    <div className={`transition-transform max-w-5xl mx-auto flex flex-col md:grid md:grid-cols-12 md:gap-6 ${shake ? 'animate-shake' : ''} md:p-6`}>
