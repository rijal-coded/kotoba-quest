...
import { BookOpen, Sparkles, Lock, ChevronDown, Search, X, Heart } from 'lucide-react';
import { Level, Page, QuestionType } from '../types';

interface WordsProps {
  levels: Level[];
  onNavigate: (page: Page) => void;
  onToggleFavorite: (levelId: string, wordIndex: number) => void;
}
...
export const Words = ({ levels, onNavigate, onToggleFavorite }: WordsProps) => {
  ...
  const totalFavorites = levels.reduce(
    (sum, l) => sum + (l.favoriteWordIndices ?? []).length,
    0
  );
  ...
  return (
    <div className="min-h-[80vh] p-4 md:p-6 pb-24 md:pb-6 space-y-4">
      ...
      {/* Stats + legend + favorites toggle row */}
      <motion.div
        initial={{ opacity: 0, y: 8 }}
        animate={{ opacity: 1, y: 0 }}
      ...
      >
        ...
        {totalFavorites > 0 && (
          <div className="kawaii-card px-3 py-1.5 text-xs text-text-secondary shrink-0 flex items-center gap-1">
            <Heart className="w-3 h-3 text-danger fill-danger" />
            <span className="font-bold text-danger">{totalFavorites}</span>
          </div>
        )}
        ...
        <button
          ...
          className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all border shrink-0 ${
            showFavoritesOnly
              ...
              : 'bg-bg-surface-alt border-border text-text-secondary hover:border-danger/30 hover:text-danger'
          }`}
        >
          {showFavoritesOnly ? 'Favorit Saja' : 'Semua Kata'}
        </button>
      ...
      {showFavoritesOnly && totalFavorites === 0 && (
        <motion.div
          initial={{ opacity: 0, y: 8 }}
          animate={{ opacity: 1, y: 0 }}
          className="text-center py-12 space-y-3"
        >
          <Heart className="w-10 h-10 text-text-secondary/30 mx-auto" />
          <p className="text-text-secondary text-sm">
            Belum ada kata favorit.
          </p>
          <p className="text-text-secondary/60 text-xs">
            Tandai kata dengan menekan <Heart className="w-3 h-3 inline text-danger" /> untuk menyimpannya di sini
          </p>
        </motion.div>
      )}
      ...
      {!(showFavoritesOnly && totalFavorites === 0) && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
          {levels.map((level, levelIdx) => {
            ...
            const favSet = new Set(level.favoriteWordIndices ?? []);
            const isExpanded = isSearching || showFavoritesOnly || expandedLevels.has(level.id);
            ...
            // Filter words when searching or showing favorites
            const wordIndices = level.words
              .map((word, idx) => ({ word, idx }))
              .filter(({ word, idx }) => {
                // Favorites filter
                if (showFavoritesOnly && !favSet.has(idx)) return false;
                // Search filter
            ...
            if ((isSearching || showFavoritesOnly) && wordIndices.length === 0) return null;
                  ...
                  {isExpanded && (
                    ...
                    >
                      {wordIndices.map(({ word, idx }) => {
                        ...
                        const isFavorite = favSet.has(idx);
                          ...
                          return (
                            ...
                            >
                              <div className="flex items-start justify-between gap-3">
                                {/* Favorite button */}
                                <motion.button
                                  onClick={() => onToggleFavorite(level.id, idx)}
                                  whileTap={{ scale: 1.3 }}
                                  className="mt-1 shrink-0 outline-none"
                        ...
                        // Locked word placeholder (hidden in favorites-only mode)
      ...
      {isSearching && levels.every(level => {
        const favSet = new Set(level.favoriteWordIndices ?? []);
        const wordIndices = level.words.filter((word, idx) => {
          if (showFavoritesOnly && !favSet.has(idx)) return false;
          return (
            word.japanese.toLowerCase().includes(q) ||
            (word.kanji && word.kanji.toLowerCase().includes(q)) ||
